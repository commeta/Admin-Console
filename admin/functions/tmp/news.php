<?php
/*
CREATE TABLE IF NOT EXISTS `md_news` (
...
if(isset($_POST['ajax'])){	// Менеджер обработки ajax запросов
	...
	if(isset($_POST['oper'])){ // Операции
		if($_POST['oper'] == 'add_url'){ // Добавление url
			$id = $db->insert ('md_news', array('friendly_url'=>"new_url_".time(), 'meta_title'=>"Новый URL ".date("d.m.y H:i:s") ));
			$_POST['id']= $id;
		...
		if($_POST['oper'] == 'load_url'){ // Загрузка значения
			...
			$db->where('meta_id', $id );
			$friendly_url = $db->getOne('md_news');
		...
		if($_POST['oper'] == 'get_table'){ // Получить таблицу импорта
			...
			$md_news= $db->get("md_news", null, ['meta_id','meta_h1']);
			
			die(json_encode(array('status'=>'Таблица получена.','css_class'=>'success', 'import'=>$md_news)));
		}
		...
		if($_POST['oper'] == 'del_url'){ // Удалить
			...
			$id = $db->escape($_POST['id']);
			$db->where('meta_id', $id);
			if($db->delete('md_news')) die(json_encode(array('status'=>'URL удален.','css_class'=>'success' )));
...
function AllTables(){
	$('#datatable-main').dataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "/admin/functions/news.php?get_table=ajax",
		"aaSorting": [[ 0, "asc" ]],
